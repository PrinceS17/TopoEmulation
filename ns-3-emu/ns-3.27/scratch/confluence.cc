...
/* 
This program is used to simulate the ITZ topology, random generated target
flows with log normal distributed leaf bandwidth & random cross traffic, given
the number of flows, flow info description and mode (which is used to get 
ground truth). It should be mainly used with confluentSim.py, which will 
pass the XML topology and flow info. 
*/
...
NS_LOG_COMPONENT_DEFINE ("Confluence");
...
Ptr<Node> findNext (Ptr<Node> cur, Ipv4Address destNetwork, FNSSSimulation sim)
{
	...
	for (uint32_t i = 0; i < gr->GetNRoutes (); i ++)
	{
		...
		{
            NS_LOG_INFO (" -- Correct route found: gw " << route->GetGateway ());
			NS_LOG_INFO (" -- Target: " << destNetwork << " Dest: " << route->GetDest () << " (" << route->GetDestNetwork () \
                << "): Mask " << mask << ", GW " << route->GetGateway ());
...
// write the final flow rates to file at the end of simulation
void writeFlowRate (string fname, vector<vdouble> rates)
{
    ofstream fout (fname, ios::out);
    double avgRate;
    for (uint32_t i = 0; i < rates.size (); i ++)
    {
        if (abs(rates[i][1] - rates[i][0]) < 0.3 * rates[i][1])
            avgRate = (rates[i][0] + rates[i][1]) / 2;
        else avgRate = max(rates[i][0], rates[i][1]);           // remove smaller unstable state rate
        fout << avgRate << endl;
    }
    fout.close ();

    NS_LOG_DEBUG ("Average sample rate collected and written to file.");
    NS_LOG_INFO (" - rate: " << avgRate << " kbps, written to: " << fname);
}

// schedule the fetch of each avg rate from rate monitors, idx: [flow No., measure No., abs/period No.]
void scheduleGetAvg (vint idx, Ptr<RateMonitor> mon, uint32_t nFlow, uint32_t mode)
{
    // &RateMonitor::getAvgRate, mons[i], rawRates[i][j], nFlow
    double rate;
    mon->getAvgRate (rate);
    cout << " ---> rate " << idx[0] << ". " << idx[1] << ": " << rate << endl;
    rates[idx[0]][idx[1]] = rate;
    bool writeCond = (mode == 1 && idx[0] == nFlow - 1 && idx[1] == 1) ||
                    (mode == 2 && idx[1] == 1 && (
                        (idx[0] == nFlow - 2 && idx[2] == nFlow - 1) ||
                        idx[0] == nFlow - 1 ));

    string name = mode == 1? fname : fnames[idx[2]];
    if (writeCond) Simulator::Schedule (Seconds (0), &writeFlowRate, fnames[idx[2]], rates);
}
...
int main (int argc, char *argv[])
{
    ...
    LogComponentEnable ("Confluence", LOG_LEVEL_INFO);      // main level: debug; detail: info
    LogComponentEnable ("FNSSSimulation", LOG_LEVEL_ALL);
    LogComponentEnable ("FNSSEvent", LOG_LEVEL_INFO);
    LogComponentEnable ("RateMonitor", LOG_DEBUG);
    ...
    // build topology: [get i-th node: sim.getNode(nids[i]) ]
    ...
    {
        Ptr<Node> tmp = sim.getNode (nids[i]);
        NS_LOG_INFO (" -- " << i << " --> " << nids[i] << ": id " << tmp->GetId ());
    }
    ...
    */
    ...
    for (auto it = edges.begin (); it != edges.end (); it ++, i ++)
    {
        ...
        NS_LOG_DEBUG (" - [" << end[0] << ", " << end[1] << "] -> " << i << ", " << linkBw[i] << " Mbps");
    ...
    printFlowRate (flowRate);

    // output & write to file
    stringstream ss;
    for (uint32_t i = 0; i < nFlow; i ++)           // we only care our target flows
        ss << i << " " << bnLabel[i] << endl;
    NS_LOG_DEBUG ("Bottleneck result <flow bottleneck-link>: \n" << ss.str ());
	...
	{	
		vint id = {mid, i};
		if (i < nFlow)          // mode 0: don't need record the data
		{
			Ptr<MiniBox> mnbox = CreateObject <MiniBox, vint> (id);
			mnboxes.push_back (mnbox);
			mnbox->install (txEnd.Get (i), txEndDev.Get (i));
			mnbox->start (Seconds (0.01));
			mnbox->stop (Seconds (tStop));
		}

        if (!isCross && i >= nFlow) break;

        Ptr<RateMonitor> mon = CreateObject <RateMonitor, vint, bool> (id, true);
        mons.push_back (mon);
		if (i < nFlow) mon->install (rxEndDev.Get (i));
		else mon->install (crEndDev.Get (i - nFlow));           // record cross traffic or not?
