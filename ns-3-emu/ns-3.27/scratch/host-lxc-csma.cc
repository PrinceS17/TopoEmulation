...
main (int argc, char *argv[])
{
  ...
  Ipv4AddressHelper ipv4Left ("10.1.1.0", "255.255.255.0");
  ...
  Ipv4AddressHelper ipv4Right ("10.1.3.0", "255.255.255.0");
  ...
  // add a new branch of host tap
  NodeContainer host;
  host.Create (2);
  NetDeviceContainer hostDev = csma.Install (host);
  InternetStackHelper stack;
  stack.Install (host);
  Ipv4AddressHelper ipv4Host ("10.1.5.0", "255.255.255.0");
  Ipv4InterfaceContainer ifcHost = ipv4Host.Assign (hostDev);
  TapBridgeHelper tbHost (ifcHost.GetAddress (1));
  tbHost.SetAttribute ("Mode", StringValue ("ConfigureLocal"));
  tbHost.SetAttribute ("DeviceName", StringValue ("tap-host"));
  tbHost.Install (host.Get (0), hostDev.Get (0));
  NS_LOG_INFO ("Host tap gateway: " << ifcHost.GetAddress (1));

  // add a new branch of right node for multi-bridge test!
  NodeContainer nodesMiddle;
  nodesMiddle.Create (2);
  NetDeviceContainer devicesMiddle = csma.Install (nodesMiddle);
  InternetStackHelper internetMiddle;
  internetMiddle.Install (nodesMiddle);
  Ipv4AddressHelper ipv4Middle ("10.1.4.0", "255.255.255.0");
  Ipv4InterfaceContainer interfacesMiddle = ipv4Middle.Assign (devicesMiddle);

  TapBridgeHelper tbMiddle;
  tbMiddle.SetAttribute ("Mode", StringValue ("UseBridge"));
  tbMiddle.SetAttribute ("DeviceName", StringValue ("tap-middle"));
  tbMiddle.Install (nodesMiddle.Get (1), devicesMiddle.Get (1));
  NS_LOG_INFO ("Middle tap gateway: " << interfacesMiddle.GetAddress (0));
  ...
  NetDeviceContainer devMid = p2p.Install (nodesLeft.Get (1), nodesMiddle.Get (0));
  NetDeviceContainer devHost = p2p.Install (host.Get (1), nodesLeft.Get (1));
...
//   apps.Start (Seconds (1.0));
  ...
  csma.EnablePcapAll ("host-lxc-csma", false);
