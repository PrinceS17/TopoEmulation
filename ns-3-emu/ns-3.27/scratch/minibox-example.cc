...
main (int argc, char *argv[])
{
  ...
  for (uint32_t i = 0; i < nFlow; i ++, vport += 10)
  {
    Flow flow (nodes.Get (0), nodes.Get (1), leftAddr, rightAddr, clientRate, {0, tStop}, vport);
    flow.build (to_string(clientRate) + "bps");
    flow.setOnoff ();
    // flow.setOnoffByLink ("ds");     // test new API
    flow.setOnoffByLink ("main", interface.GetAddress (1));
