...
// change link bandwidth (ppp or csma) in the middle of simulation
void changeLinkBandwidth (NetDeviceContainer dev, uint32_t bw, bool useP2p=true)
{
    if (useP2p)
    for (uint32_t i:{0, 1})
    {
        Ptr<PointToPointNetDevice> pdev = DynamicCast<PointToPointNetDevice> (dev.Get (i));
        pdev->SetDataRate (DataRate (bw));
    }
    else
    {
...
uint32_t getTcpSizeEth (Ptr <const Packet> p)
{
  Ptr<Packet> pktCopy = p->Copy();
  EthernetHeader eH;
  Ipv4Header ipH;
  TcpHeader tcpH;
  pktCopy->RemoveHeader(eH);
  pktCopy->RemoveHeader(ipH);
  pktCopy->RemoveHeader(tcpH);
...
uint32_t getTcpAckNoEth (Ptr <const Packet> p)      // depends on the MAC layer of sender
{
  Ptr<Packet> pcp = p->Copy ();
  EthernetHeader ethH;
  Ipv4Header ipH;
  TcpHeader tcpH;
  pcp->RemoveHeader (ethH);
  pcp->RemoveHeader (ipH);
  pcp->RemoveHeader (tcpH);
