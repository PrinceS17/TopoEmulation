...
NetDeviceContainer Flow::build (string rate, string dsRate, bool useCsma)       // create node, add link, install stack, assign address
{
    ...
    NetDeviceContainer clientDev;
    if (useCsma) clientDev = csma.Install (client);
    else clientDev = p2p.Install (client);        // needed for simulation, emu will fail
    ...
    if (useCsma) serverDev = dsCsma.Install (server);
    else serverDev = dsP2p.Install (server);
...
void Flow::setLocalBridge (string tap_left, string tap_right)
{
    TapBridgeHelper tbh (interface.GetAddress (1));
    tbh.SetAttribute ("Mode", StringValue ("ConfigureLocal"));
    tbh.SetAttribute ("DeviceName", StringValue (tap_left));
    tbh.Install (endNode.Get (0), device.Get (0));
    ...
    tbh.SetAttribute ("DeviceName", StringValue (tap_right));
    tbh.Install (endNode.Get (1), device.Get (3));
    NS_LOG_INFO ("Tap (local) bridge set: " << tap_left << " : " << interface.GetAddress (0) <<\
        ", " << tap_right << " : " << interface.GetAddress (3) << endl);
}
