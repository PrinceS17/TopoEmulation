...
NS_OBJECT_ENSURE_REGISTERED (RateMonitor);

TypeId RateMonitor::GetTypeId (void)
{
    static TypeId tid = TypeId ("ns3::RateMonitor")
        .SetParent <Object> ()
        .SetGroupName ("RateMonitor")
        .AddConstructor <RateMonitor> ()
        .AddTraceSource ("highRate",
                     "High rate meaning flow starts",
                     MakeTraceSourceAccessor (&RateMonitor::m_highRate),
                     "ns3::TracedValueCallback::Bool");
    return tid;
}

RateMonitor::RateMonitor (vector<uint32_t> id, bool useRx, double period): m_id(id), m_period(period), \
    m_bytes (0), m_pktSize (1500), m_useRx (useRx), m_avgRate (0), m_highRate (false), m_started (false)
{
...
void RateMonitor::connect ()
{
    ...
    if (m_useRx)
        ...
        m_device->TraceConnectWithoutContext ("PhyRx", MakeCallback (&RateMonitor::onMacRx, this));   
}
...
void RateMonitor::disconnect ()
{
    ...
    if (m_useRx)
        ...
        m_device->TraceDisconnectWithoutContext ("PhyRx", MakeCallback (&RateMonitor::onMacRx, this));
}
